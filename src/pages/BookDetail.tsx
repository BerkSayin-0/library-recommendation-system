...
import { getBook, getReadingLists, addBookToReadingList } from '@/services/api';
        ...
        >
          <div className="space-y-4">
            <p className="text-gray-600">
              Select a list to add <b>{book.title}</b>:
            </p>
            ...
            {readingLists.length > 0 ? (
              <select
                ...
                className="w-full p-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-violet-500 focus:border-violet-500 outline-none transition-all"
              >
                {readingLists.map((list) => (
                  <option key={list.id} value={list.id}>
                    {list.name} ({list.bookIds?.length || 0} books)
                  </option>
                ))}
              </select>
            ) : (
              <div className="text-center p-4 bg-red-50 text-red-600 rounded-lg">
                <p>No lists found. Please create one first.</p>
              </div>
            )}
            ...
            <div className="flex justify-end gap-3 mt-6">
              ...
              <Button
                ...
                disabled={isAdding || readingLists.length === 0}
              >
